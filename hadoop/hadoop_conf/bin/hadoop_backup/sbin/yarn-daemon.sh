...
##
printTrace "Entering yarn-daemon.sh. $*"
...
printTrace "log=$YARN_LOG_DIR/yarn-$YARN_IDENT_STRING-$command-$HOSTNAME.out"
printTrace "pid=$YARN_PID_DIR/yarn-$YARN_IDENT_STRING-$command.pid"
...
case $startStop in
  ...
  (start)
    ...
    printTrace "start yarn, YARN_MASTER=${YARN_MASTER}"
